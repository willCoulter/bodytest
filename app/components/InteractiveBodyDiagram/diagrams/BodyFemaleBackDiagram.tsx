...
export const BodyFemaleBackDiagram = forwardRef<SVGSVGElement, Props>(function BodyFemaleBackDiagram(
  {
    mode,
    selectedZones,
    onZoneClick,
  ...
  function getFill(selectionId: string, baseColor: string, slug: string): string {
  ...
  function renderZonePaths(zone: (typeof bodyFemaleBack)[0]) {
    ...
    return pathGroups.map(({ side, paths }) => {
      ...
      const fill   = getFill(selId, zone.color, zone.slug);
  ...
  return (
    <div style={{ position: 'relative', height, width: '100%' }}>
      ...
      >
        <g>{bodyFemaleBack.map((zone) => renderZonePaths(zone))}</g>
